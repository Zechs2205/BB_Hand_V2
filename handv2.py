#!/usr/bin/env python3
# hand_control_serial_final.py
#
# • Palm-relative abduction/adduction (+90 ° offset) for Index … Pinky
# • Flexion 0–180 ° with per-finger floor / ceiling thresholds
...
# Serial link (set SERIAL_PORT=None to disable)
SERIAL_PORT = "COM11"        # Windows "COMx",  Linux "/dev/ttyUSB0"
BAUD_RATE   = 1000000
...
AB_MIN  = [-30, -30, -30, -30]
AB_MAX  = [ 30,  30,  30,  30]
AB_REV  = [True, True, True, True]

OFFSET_DEG = 90.0           # +90 ° so “straight finger” = 90

AB_DISABLE_TOL = 2.0        # |flex-90| ≤ 2 ° → zero ab/ad
# ────────────────────────────────────────────────────────────────
...
# ───────────────── LANDMARK INDICES ─────────────────────────────
FINGER_MCP   = [5, 9, 13, 17]            # I  M  R  P
FINGER_PIP   = [6,10,14,18]
FLEX_FINGERS = [(1,4), (5,8), (9,12), (13,16), (17,20)]  # T I M R P
...
def abduction_angles(pts):
    ...
    # thumb ab/ad (simple MCP-IP projection)
...
def flexion_angles(pts):
    ...
    for k,(mcp,tip) in enumerate(FLEX_FINGERS):
        ...
        # thresholds
        if FLEX_TLO[k] is not None and ang <= FLEX_TLO[k]: ang = FLEX_MIN[k]
        if FLEX_THI[k] is not None and ang >= FLEX_THI[k]: ang = FLEX_MAX[k]
        vals.append(ang)
    return vals
...
def window_title(ab_rev_flags):
    tag = '|'.join(f'{n[0]}{"-" if r else "+"}'
                   for n,r in zip(["I","M","R","P"], ab_rev_flags))
...
while cap.isOpened():
    ...
    if res.multi_hand_landmarks:
        for hl in res.multi_hand_landmarks:
            ...
            # compute angles
            abd, thumb_ab = abduction_angles(pts)   # list(4), scalar
            flex = flexion_angles(pts)              # list(5)
            ...
            packet_str = "A " + " ".join(map(str,pkt)) + "\n"
            if ser: ser.write(packet_str.encode())
